...
페이지 맨 아래의 「수정 모드」(또는 F2)를 켜면 말풍선을 끌어서 옮기고, 네 모서리 손잡이로 크기를, 위쪽 손잡이로 기울기를 바꿀 수 있습니다. 말풍선을 더블클릭하면 글을 고칠 수 있고, 그 안에서 아래 단축키가 동작합니다. 그림마다 빨간 점선(기준선)이 그림 위 끝을 나타내며, 점선을 끌면 기준선만, 왼쪽 이름표를 끌면 대사까지 함께 옮겨집니다. 옮긴 기준선은 YAML에 `top`으로 기록됩니다. 고친 내용은 「YAML 복사」로 복사해 리포지토리에 반영합니다.
...
| b (누른 채로)         | 마우스 위치를 왼쪽 위 모서리로 상자를 그리기       |
| s (누른 채로)         | 마우스 움직임만큼 폭·높이 조정                     |
| Esc                   | 집기·b·s 취소                                      |
