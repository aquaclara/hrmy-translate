...
페이지 맨 아래의 「수정 모드」(또는 F2)를 켜면 말풍선을 끌어서 옮기고, 오른쪽 아래 손잡이로 크기를, 위쪽 손잡이로 기울기를 바꿀 수 있습니다. 말풍선을 더블클릭하면 글을 고칠 수 있고, 그 안에서 아래 단축키가 동작합니다. 고친 내용은 「YAML 복사」로 복사해 리포지토리에 반영합니다.
