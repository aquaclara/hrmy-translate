...
두 가지가 있습니다. 둘 다 원본 사이트를 그대로 보면서 옆에 한국어 번역을 표시하며, 이미지 내 텍스트 번역만을 지원합니다. 일반 텍스트는 브라우저의 번역 기능을 사용해주세요.

### 웹 브라우저 안의 브라우저

[첫 페이지](./)의 주소창에 `dka-hero.me`를 입력하면 원본 사이트가 그 안에 열리고, 번역이 있는 페이지에서는 옆에 번역 서랍이 나타납니다. 설치가 필요 없고 휴대폰에서도 동작합니다.

### 브라우저 확장프로그램

설치하면 [dka-hero.me] 방문 시 만화 오른쪽에 번역이 표시됩니다. 번역을 직접 수정하는 [수정 모드](editable-mode.html)도 확장프로그램에서만 쓸 수 있습니다.
...
크롬 웹 스토어에서는 더 이상 배포하지 않습니다. 압축 해제된 확장프로그램으로 직접 불러옵니다.

1. [Releases](https://github.com/aquaclara/hrmy-translate/releases)에서 `extension.zip`을 받아 압축을 풉니다.
2. 크롬에서 `chrome://extensions`를 열고 오른쪽 위의 **개발자 모드**를 켭니다.
3. **압축해제된 확장 프로그램을 로드합니다**를 누르고 압축을 푼 폴더를 선택합니다.
4. [dka-hero.me]을 방문해 보고 싶은 만화를 봅니다.
